...
報到時間通常就等於行程本身的排程時間，記兩次沒有意義。這個欄位改為記錄交通工具的
出發時間（班機起飛、列車發車），編輯畫面標籤與卡片摘要一併更新
（`報到 11:05` → `出發 11:05`）。

資料層完全沒動：envelope 的鍵名仍是 `checkin`，既有資料不需要搬遷，舊資料會直接以
新標籤顯示。`js/itinerary.js` 有註解說明鍵名與語意不一致的原因。

已知取捨：交通類型的行程 `time` 欄位標籤本來就是「出發時間」，所以編輯畫面上會同時出現
兩個「出發時間」。這是刻意接受的 —— 只改文字、不動資料結構與其他欄位。
