...
// version: 20260910.1 complete backend; legacy itinerary image columns are left untouched.
/************ CONFIG ************/
const BACKEND_VERSION = '20260910.1';
...
const REQUIRED_TYPE_HEADERS = {
  itinerary: ['type'],
  trips: ['shared_wallet_enabled', 'foreign_currency', 'foreign_to_twd_rate'],
  expenses: ['currency']
};
...
function ensureBackendSupportSheets_(ss) {
  ...
  if (expensesSheet) ensureTypeHeaders_(expensesSheet, 'expenses');
}
...
function handleActionGET(ss, action, type, e) {
  ...
  if (action === 'add' || action === 'edit') {
    ...
    if (type === 'trips' && data && data.id) clearTripDataCache_(data.id);
    return out;
  }
...
function normalizeExpenseCurrency_(value) {
  const currency = String(value == null ? '' : value).trim().toUpperCase();
  if (!currency) return 'TWD';
  if (currency === 'TWD' || currency === 'FOREIGN') return currency;
  throw new Error('invalid currency');
}

function readTripCurrencySettings_(ss, tripId) {
  if (!tripId) throw new Error('missing tripId');
  const sheet = ss.getSheetByName('trips');
  if (!sheet) throw new Error('sheet not found: trips');
  ensureTypeHeaders_(sheet, 'trips');
  const trip = findRowById_(sheet, tripId);
  if (!trip) throw new Error('trip not found');

  const currency = String(trip.foreign_currency || '').trim().toUpperCase();
  const rate = Number(trip.foreign_to_twd_rate);
  if (currency && !/^[A-Z]{3}$/.test(currency)) throw new Error('invalid foreign currency code');
  if (currency && (!isFinite(rate) || rate <= 0)) throw new Error('invalid foreign exchange rate');
  if (!currency && rate) throw new Error('foreign currency code is required');
  return { currency: currency, rate: isFinite(rate) && rate > 0 ? rate : 0 };
}

function validateForeignTransaction_(currency, settings) {
  const normalized = normalizeExpenseCurrency_(currency);
  if (normalized === 'FOREIGN' && (!settings || !settings.currency || !isFinite(settings.rate) || settings.rate <= 0)) {
    throw new Error('foreign currency settings are required');
  }
  return normalized;
}

function convertBackendAmountToTwd_(amount, currency, settings) {
  const value = Number(amount) || 0;
  const normalized = validateForeignTransaction_(currency, settings);
  return normalized === 'FOREIGN' ? value * settings.rate : value;
}
...
function normalizeSharedWalletTransaction_(value, existing, currencySettings) {
  const source = value || {};
  ...
  const tripId = String(source.trip_id || source.tripId || previous.trip_id || '').trim();
  const settings = currencySettings || readTripCurrencySettings_(SpreadsheetApp.openById(SPREADSHEET_ID), tripId);
  const currency = validateForeignTransaction_(source.currency != null ? source.currency : previous.currency, settings);
    ...
    .map(item => {
      try {
        return {
          ...
          currency: normalizeExpenseCurrency_(item.currency),
...
function sharedWalletSignedAmount_(transaction, settings) {
  const amount = convertBackendAmountToTwd_(transaction && transaction.amount, transaction && transaction.currency, settings);
...
function handleSharedWalletMutation_(ss, contents) {
  ...
  if (action === 'shared_wallet_add') {
    const settings = readTripCurrencySettings_(ss, tripId);
    const transaction = normalizeSharedWalletTransaction_(requestData, null, settings);
    const currentBalance = readSharedWalletTransactions_(sheet, transaction.trip_id)
      .reduce((sum, item) => sum + sharedWalletSignedAmount_(item, settings), 0);
    if (currentBalance + sharedWalletSignedAmount_(transaction, settings) < 0) throw new Error('insufficient wallet balance');
  ...
  if (action === 'shared_wallet_edit') {
    ...
    const settings = readTripCurrencySettings_(ss, tripId || found.item.trip_id);
    const transaction = normalizeSharedWalletTransaction_(requestData, found.item, settings);
    const currentBalance = readSharedWalletTransactions_(sheet, transaction.trip_id)
      .reduce((sum, item) => sum + sharedWalletSignedAmount_(item, settings), 0);
    const projectedBalance = currentBalance - sharedWalletSignedAmount_(found.item, settings) + sharedWalletSignedAmount_(transaction, settings);
  ...
  if (action === 'shared_wallet_delete') {
    ...
    const settings = readTripCurrencySettings_(ss, existingTripId);
    const projectedBalance = readSharedWalletTransactions_(sheet, existingTripId)
      .reduce((sum, item) => sum + sharedWalletSignedAmount_(item, settings), 0) - sharedWalletSignedAmount_(found.item, settings);
    if (projectedBalance < 0) throw new Error('insufficient wallet balance');
...
function validateTripCurrencySettings_(currencyValue, rateValue) {
  const currency = String(currencyValue == null ? '' : currencyValue).trim().toUpperCase();
  const rate = Number(rateValue);
  if (!currency && (!rateValue || rateValue === 0)) return { currency: '', rate: 0 };
  if (!/^[A-Z]{3}$/.test(currency)) throw new Error('invalid foreign currency code');
  if (!isFinite(rate) || rate <= 0) throw new Error('invalid foreign exchange rate');
...
function addOrEditRow_(ss, type, action, data) {
  ...
  if (type === 'expenses') {
    const existing = action === 'edit' ? findRowById_(sheet, data && data.id) : null;
    const tripId = String((data && data.trip_id) || existing?.trip_id || '').trim();
    const settings = readTripCurrencySettings_(ss, tripId);
    data = Object.assign({}, data || {}, {
      currency: validateForeignTransaction_(data && data.currency, settings)
    });
  }

  if (type === 'trips' && (Object.prototype.hasOwnProperty.call(data || {}, 'foreign_currency')
    || Object.prototype.hasOwnProperty.call(data || {}, 'foreign_to_twd_rate'))) {
    const existing = action === 'edit' ? findRowById_(sheet, data && data.id) : null;
    const settings = validateTripCurrencySettings_(
      data && data.foreign_currency != null ? data.foreign_currency : existing?.foreign_currency,
      data && data.foreign_to_twd_rate != null ? data.foreign_to_twd_rate : existing?.foreign_to_twd_rate
    );
    data = Object.assign({}, data || {}, {
      foreign_currency: settings.currency,
      foreign_to_twd_rate: settings.rate
    });
  }
