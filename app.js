...
createApp({
  setup() {
    ...
    const APP_VERSION = window.TRAVEL_CONFIG?.APP_VERSION || '20260913.1';
    ...
    const manualSync = async () => {
      ...
      try {
        ...
        const hasPendingWrites = pendingSyncQueue.value.length > 0;
        if (hasPendingWrites) {
          syncStatus.value = 'error';
          syncMessage.value = pendingQueueFailureMessage(pendingSyncQueue.value);
        ...
        return Boolean(refreshed && !hasPendingWrites);
      } catch (err) {
