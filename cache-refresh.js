...
(function () {
  const VERSION = (window.TRAVEL_CONFIG && window.TRAVEL_CONFIG.APP_VERSION) || '20260915.2';

  async function clearBrowserCaches() {
    try {
      if (!('caches' in window)) return;
      const keys = await caches.keys();
      await Promise.all(keys.filter(key => key !== 'travel-prep-images-v1').map(key => caches.delete(key)));
    } catch (err) {
