...
// 20260704.6：移除整份覆蓋式 prep_checklist_save，避免手機舊 localStorage 覆蓋 Google Sheet。
// 20260704.6：離線時只允許查看，不允許新增、編輯、刪除、勾選或清空。
// 20260704.6：新增 / 編輯 / 刪除改成樂觀式局部 UI；背景排隊寫入，不再成功後整面重畫。
(function () {
  ...
  async function loadFromSheet(options = {}) {
    ...
    try {
      ...
      if (data && data.status === 'success') {
        // 20260704.6：資料庫是主資料。讀取時用 Google Sheet 取代前端快取，但只刷新準備清單區塊。
